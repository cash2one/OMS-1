...
class OrderService:
    ...
    def list(self, user_id, kwargs):
        ...
        elif kwargs.get('express_number'):
            query = query.filter(express_number__contains=kwargs.get('express_number'))
        ...
        if kwargs.get('is_locked') is not None:
            ...
            if is_locked:
                query = query.filter(Q(order_mark=40) | Q(is_locked=True))
        ...
        paginator = Paginator(query.filter(user_id=user_id),
                              page_size)
        return paginator
    ...
    # 编辑订单
    @transaction.atomic
    def update(self, user_id, order_id, data):
        self.update_base_info(user_id=user_id, order_id=order_id, data=data)
        if 'order_details' in data.keys():
            self.update_sku_info(user_id=user_id,
                                 order_id=order_id,
                                 data=data)
        return self.get(user_id=user_id, order_id=order_id)

    # 路由订单
    # TODO 使用numpy,构建多维矩阵
    def route_order(self, user_id, order_id, token):
        self.order = self.get(user_id=user_id, order_id=order_id)
            ...
            for inventory in inventories:
                ...
                if inventory.warehouse_id not in self.warehouses:
                    ...
                    self.warehouses[inventory.warehouse_id] =\
                        self.get_warehouse(inventory.warehouse_id, token=token)
                    # self.warehouses.\
                    #     setdefault(inventory.warehouse_id,
                    #             self.get_warehouse
                    #             (inventory.warehouse_id, token=token))
        # 可选仓库组合和一个组合因子
        # 1：不拆单
        # 2：拆两单
        # ......
        ...
        for sku_id in self.sku_required_quantity.keys():
            ...
            if required_quantity <= a:
                ...
                while required_quantity:
                    ...
                    result[item[0]][sku_id] = required_quantity - item[1] if\
                        item[1] <= required_quantity else required_quantity
                    required_quantity -= required_quantity - item[1] if\
                        item[1] <= required_quantity else required_quantity
            else:
                tag = False
                break
    ...
    # 锁定订单
    def lock_order(self, user_id, order_id, lock_reanson):
        order = Order.objects.get(user_id=user_id, id=order_id)
    ...
    # 订单解锁
    def unlock_order(self, user_id, order_id):
        order = Order.objects.get(user_id=user_id, id=order_id)
    ...
    # 订单撤回
    def withdraw_order(self, user_id, order_id):
        order = Order.objects.get(user_id=user_id, id=order_id)
        # TODO 撤回逻辑
        return order
    ...
    # 编辑商品信息
    def update_sku_info(self, user_id, order_id, data):
        create_order_details = []
